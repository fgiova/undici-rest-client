...
*/
		...
		else if (this.IsAMock) {
			this.undiciClient = getGlobalDispatcher();
		}
		else {
					...
					(dispatch: Dispatcher["dispatch"]) => {
						return (
							...
							if (options.bypassNativeCache && options.headers) {
								(options.headers as IncomingHttpHeaders)["cache-control"] =
									"no-store";
							}
	...
	public readonly request = async <TResponseBody>(
		...
		const responseData = async (
			...
			if (isError) {
				let message = `${(typeof data === "object" && data && (("message" in data && data.message) || ("error" in data && data.error))) || rawBody}`;
		...
		const retryTimeout = async (
			...
			const retryableWithDelay =
				[429, 502, 503, 504].includes(retryResponse.statusCode) &&
				retryAfterHeader;
				...
				if (retryAfter > 0) {
					...
					if (new Date(retryAfterHeader).valueOf() > Date.now()) {
						...
						if (retryAfterDate > this.MaxRetryTimeout) {
							// biome-ignore lint/suspicious/noExplicitAny: <explanation>
							const error = await responseData(retryResponse, true) as any;
...
*/
		const result = new Promise<Dispatcher.ResponseData>(
			...
			.then(async (result) => {
						...
						if (this.isIdempotentMethod(method) && ttl !== undefined) {
							this.localCache.set(`${requestKey}#data`, data, {
								...
								size: this.localCache.maxSize
									? (data as string).constructor === String
				...
				// biome-ignore lint/suspicious/noExplicitAny: <explanation>
				const error = (await responseData(result, true)) as any;
		...
		return result as Promise<TResponseBody>;
	};
}
